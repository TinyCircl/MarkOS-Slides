...
const BUILTIN_ASSETS_DIR = join(REPO_ROOT, "assets");
const BUILTIN_PRESETS_DIR = join(REPO_ROOT, "styles", "presets");
...
async function loadBundledDeckCss(workDir) {
    return bundleCssFile(join(workDir, "styles", "index.css"));
}
...
async function copyBuiltInAssets(outputDir) {
    if (await pathExists(BUILTIN_ASSETS_DIR)) {
        await copyRenderableAssets(BUILTIN_ASSETS_DIR, outputDir, REPO_ROOT);
    }
}
